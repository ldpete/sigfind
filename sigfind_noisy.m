...
function [signal, background,data, LB, UB]=sigfind_noisy(file_name, t_um)
...
%% Smooth Data to remove effects of "noise"/scattering
...
%shoulders. We use a sufficiently large span (following estimates of Baek
%et al. 2009) to smooth the data without removing peak information.
...
rband=convhull(data(:,1),smth); % Finds the convex hull of the data
...
%% Create Array of all Data Greater than 15% of max peak height
p1_sig=find(tmp>0.15*M); % Gives indices of >15% max peak height
...
%% find the convex hull of the Raw Data to adjust LB and UB toward
rband2=convhull(data(:,1),data(:,2));
id=find([diff(rband2)]<0); % finds the upper part of the convex hull that is above the data
rband2(id(1)+1:length(rband2))=[]; %removes upper part of convex hull to leave baseline

%% Adjust LB and UB to the nearest Anchor point of the Convex Hull of the Smoothing Function
[~, timon]=min(abs(rband2-LB));
LB=rband2(timon);
[~, pumbaa]=min(abs(rband2-UB));
UB=rband2(pumbaa);
...
background=zeros(length(background_L)+length(background_R),2);
background(1:length(background_L),:)=background_L;
background(length(background_L)+1:end,:)=background_R;
%% Add any Anchor Points of Convex Hull within LB and UB to the background
for i=1:length(rband2)
    if rband2(i)>LB & rband2(i)<UB
        background(rband2(i),:)=data(rband2(i),:);
    end
end
